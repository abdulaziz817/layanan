...
const Hero = () => {
  ...
  const [fadeStage, setFadeStage] = useState("fadeIn"); // 'fadeIn' | 'fadeOut'

  useEffect(() => {
    const interval = setInterval(() => {
      setFadeStage("fadeOut"); // mulai fade out

      // tunggu animasi fade out selesai sebelum ganti teks dan fade in
      const timeout = setTimeout(() => {
        setIndex((prev) => (prev + 1) % services.length);
        setFadeStage("fadeIn");
      }, 600); // durasi fade out harus sama dengan durasi CSS transition
  ...
  // class berdasarkan tahap animasi
  const fadeClass =
    fadeStage === "fadeIn"
...
<section className="bg-white px-6 py-20 sm:py-28 min-h-screen flex items-center justify-center">
  <div className="max-w-3xl mx-auto text-center text-gray-800 mt-8">
    ...
    <div className="relative h-16 sm:h-20 overflow-hidden mb-6">
      <h2
        className={`absolute inset-0 flex items-center justify-center text-center text-2xl sm:text-4xl font-semibold text-indigo-600 transition-all duration-600 ease-in-out transform ${fadeClass}`}
