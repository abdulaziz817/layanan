...
/**
 * Update 1 row di sheet.
 * rowIndex1Based = nomor baris di Google Sheet (1-based).
 * row = array values (misal 14 kolom).
 *
 * NOTE:
 * - Kalau kamu mau selalu fix 14 kolom untuk products, pakai updateRowFixedProducts().
 */
...
/**
 * ✅ Khusus sheet products (A..N = 14 kolom)
 * Ini yang paling aman untuk fitur PUT/DELETE kamu.
 */
export async function updateRowFixedProducts(rowIndex1Based, row) {
  const spreadsheetId =
    process.env.GOOGLE_SHEETS_ID || process.env.GSHEET_ID;

  if (!spreadsheetId) throw new Error("GOOGLE_SHEETS_ID / GSHEET_ID belum diset.");

  const sheets = getSheetsClient();

  // pastikan panjang 14 kolom (A..N)
